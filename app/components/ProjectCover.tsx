...
function Waveform() {
  ...
  return (
    <svg viewBox="0 0 400 225" className="h-full w-full" aria-hidden>
      {bars.map((h, i) => (
        <rect key={i} x={38 + i * 14} y={112 - h / 2} width={5} height={h} rx={2.5} fill={i % 3 === 0 ? S : F} />
      ))}
    </svg>
  )
}
...
function DialFan() {
  ...
  return (
    <svg viewBox="0 0 400 225" className="h-full w-full" aria-hidden>
      {targets.map((y, i) => (
        <path key={i} d={`M 70 112 C 180 112, 220 ${y}, 330 ${y}`} fill="none" stroke={i % 2 ? F : S} strokeWidth={1.2} />
      ))}
...
function StreakChart() {
  ...
  return (
    <svg viewBox="0 0 400 225" className="h-full w-full" aria-hidden>
      ...
      <path d={`M 66 ${170 - 30} ${bars.map((h, i) => `L ${66 + i * 30} ${168 - h}`).join(" ")}`} fill="none" stroke={S} strokeWidth={1.2} />
    </svg>
  )
}
...
function Journal() {
  ...
  return (
    <svg viewBox="0 0 400 225" className="h-full w-full" aria-hidden>
      ...
      {lines.map(([x, w], i) => (
        <line key={i} x1={x} y1={90 + i * 20} x2={x + w} y2={90 + i * 20} stroke={i === 0 ? S : F} strokeWidth={4} strokeLinecap="round" />
      ))}
    </svg>
  )
}
...
function FlightPath() {
  ...
  return (
    <svg viewBox="0 0 400 225" className="h-full w-full" aria-hidden>
      <path d="M 60 170 Q 100 110 140 90 T 230 140 T 330 56" fill="none" stroke={S} strokeWidth={1.2} strokeDasharray="5 6" />
      {waypoints.map(([x, y], i) => (
        <g key={i}>
